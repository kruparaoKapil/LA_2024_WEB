...
## Phases
0. Baseline & safety net (in progress)
1. Scaffold `app-v21/` with PrimeNG (Aura), zoneless change detection, hash routing, functional interceptors
2. Port cross-cutting layer: Models, Services (signals + `inject()`), Interceptors (functional), Pipes, retained directives, login/layout shell
3. Build `shared/ui` PrimeNG wrapper library
4. Lazy routing skeleton
5. Port `Common`
6. Port `Settings`
7. Port `Loans`
8. Port `Accounting`
9. Port `Banking`
10. Port `HRMS`, `TDS`, remaining
11. Cutover (replace `src/`, drop legacy deps)
12. QA + performance + bundle measurement
13. Write `MIGRATION_NOTES.md` (final replacement-rationale doc)
