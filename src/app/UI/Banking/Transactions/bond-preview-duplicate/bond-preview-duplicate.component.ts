...
@Component({
  selector: 'app-bond-preview-duplicate',
  templateUrl: './bond-preview-duplicate.component.html',
  styles: []
})
export class BondPreviewDuplicateComponent implements OnInit
 {
  ...
  showdetails()
  {
    debugger;
    let date = new Date();
    this._fdrdtranscationservice.GetBondspreDetails(date).subscribe(json => {
      debugger;
      this.gridData = json;
  ...
  GetAccountDetails(DataItem) {
    ...
    this.fdTranscationDetailsComponent.BindData(DataItem.pnumappid,DataItem.pfdaccountid);
  ...
  editHandler(event) {
    ...
    let dataItem = event.dataItem.pnumappid;

    let modifiedDataItem = `'${dataItem}'`;
    this._fdrdtranscationservice.GetBondPreviewReport(modifiedDataItem).subscribe(res=>{

   
      this.Editdetails = res;
      console.log("edit details", res);

      var paarams=event.dataItem.pnumappid;
