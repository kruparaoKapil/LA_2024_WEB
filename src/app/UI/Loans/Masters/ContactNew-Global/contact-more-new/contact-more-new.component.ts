...
declare let $: any
@Component({
  selector: 'app-contact-more-new',
  templateUrl: './contact-more-new.component.html',
 styleUrls: []
})
export class ContactMoreNewComponent implements OnInit {
  ...
  // surendra
  Cadredata: any;
  // showKgmsAgentFlag: boolean = false;
  ...
  ngOnInit() {
    ...
    this.contactMoreForm = this.formbuilder.group({
      ...
      introducedname: [''],
      introducedid: [0],
      // pWealthAdvisoryCode: ['', Validators.required],
      pWealthAdvisoryCode: [''],
    ...
    if (tabname == 'refferral') {
      ...
      this._contacmasterservice.getContactDetailsReferralByID(this.ID).subscribe(json => {
        ...
        if (json.introducedid != null) {
          ...
          this.contactMoreForm.controls.pWealthAdvisoryCode.setValue(json.pWealthAdvisoryCode);
        ...
        // if (json.pcadreid != null) {
        //   this.contactMoreForm.controls.pcadreid.setValue(parseInt(json.pcadreid));
        //   this.contactMoreForm.controls.pcadrename.setValue(json.pcadrename);
        ...
        if (json.pPanNumber != null && json.pPanNumber != '') {
          ...
          if (json.pIsPanNoAvailable == true) {
            ...
            if (json.pPanNumber == "No pan") {
              ...
              //bhargavi
              // this._contacmasterservice.getEasyChitAgentCode(pan1).subscribe(res => {
              //   console.log('responce form conract more',res);

              //   let PAN = res[0].pPannumber;
              //   // let PAN='BGPPM1638G'
              //   let kgms = res[0].pAgencCode;
              //   let agentunique = res[0].pAgentuniquecode
              //   if (pan1 == PAN) {
              //     // this.showKgmsAgentFlag = true;
              //     this.contactMoreForm.controls.pKgmsagentcode.setValue(kgms)
              //     this.contactMoreForm.controls.pWealthAdvisoryCode.setValue(agentunique)
              //   }
              // })
  ...
  // ✅ added safety checks (no structure change)
  ...
  savesupplier() {
    ...
    if (confirm("Do you want to save ?")) {
      ...
      this._contacmasterservice.SaveContactSupplier(Supplierdata).subscribe(res => {
  ...
  uploadAndProgressImage(event: any, files) {
    debugger;
    if (!this.validateFile(event.target.value
      ...
      this._commonService.fileUpload(formData).subscribe(data => {
        ...
        this.contactMoreForm['controls']['pPhoto'].setValue(data[0]);
        this.contactMoreForm['controls']['pReferralimagepath'].setValue(data[1]);
      })
    }
  ...
  validateSaveDeatails(control: FormGroup): boolean {
    let isValid = true;
    let kgms = this.contactMoreForm.controls.pKgmsagentcode.value;
let agent = this.contactMoreForm.controls.pWealthAdvisoryCode.value;
  ...
  saveReferral() {
    debugger;
    // this.contactMoreForm.controls.pcadreid.setValue(0);
    // this.contactMoreForm.controls.pcadrename.setValue('');
  ...
  // if (!kgms || kgms.trim() === '' || !agent || agent.trim() === '') {
  //   this._commonService.showErrorMessage("Please add agent in KGMS");
  //   return;
  // }
 if (!kgms || kgms.trim() === '' || !agent || agent.trim() === '') {
    this._commonService.showWarningMessage("Please add agent in KGMS Application");
    return;
  }
  ...
  //  Hidden For Commercial Building Purpose


  // SelectCadreName(event: any) {

  //   if (event.target.value != '') {
  //     this.contactMoreForm.controls.pcadrename.setValue(event.target.options[event.target.selectedIndex].text);
  //     this.contactMoreForm.controls.pcadreid.setValue(Number(event.target.value));
  //   }
  // }

  // getCadreNames() {
  //   debugger;
  //   this._contacmasterservice.getCadreNamesFromService().subscribe(data => {
  //     debugger;
  //     this.Cadredata = data;
  //   });
  // }
