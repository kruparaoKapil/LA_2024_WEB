...
@Component({
  selector: 'app-bank-details-new-global',
  templateUrl: './bank-details-new-global.component.html',
  styles: []
})
export class BankDetailsNewGlobalComponent implements OnInit {
  BankForm: FormGroup;
  ...
  ngOnInit() {
    ...
    this._commonService._GetBankUpdate().subscribe(data => {
      ...
      if (data.length > 0) {
        ...
        this.BankForm['controls']['pbanktypeTypeChk'].setValue(datatabledisplay2[0].gBankAccountNumber);
      }
      //this.bankdetailslist = data;
    });
    debugger;
  ...
  branchnamesName_Change(event) {
    debugger
    this.BankForm['controls']['bankcontrols']['controls']['pBankifscCode'].setValue(event.pIfsccode);
  }
  ...
  deleteBankDeatails(row, index): void {
    debugger;
    try {
      if (row.ptypeofoperation != 'CREATE') {
        let deleterow: [] = row;
        this.deletedBankDetails = [...this.deletedBankDetails, ...deleterow]
        this.deletedBankDetails = this.deletedBankDetails.filter(data => data.ptypeofoperation = 'DELETE')
      ...
      if (this.bankdetailslist.length >= 1) {
        this.BankForm['controls']['pbanktypeTypeChk'].setValue(this.bankdetailslist[0].gBankAccountNumber);
        this.bankdetailslist[0].pIsprimaryAccount = true;
        this.bankdetailslist[0].ptypeofoperation = 'UPDATE';
      }
  ...
  AddorUpdatedata() {
    ...
    if (this.checkValidations(Bankcontrols)) {
      ...
      if (accountno) {
        if (this.buttontext == 'Update') {
          for (var i = 0; this.bankdetailslist.length > i; i++) {
            if (this.bankdetailslist[i].pBankAccountNo == accountno.trim() && i != this.Bankindex) {
          ...
          for (var i = 0; this.bankdetailslist.length > i; i++) {
            if (this.bankdetailslist[i].pBankAccountNo == accountno.trim()) {
              accountnovalidate = false;
            }
          }
        }
      ...
      if (accountnovalidate == true) {
        debugger;
        if (this.buttontext == 'Update') {
          if (Bankcontrols['controls']['precordid'].value == 0) {
  ...
  changePriority(event, row, rowIndex, group) {
    ...
    try {
      for (var i = 0; this.bankdetailslist.length > i; i++) {
        if (this.bankdetailslist[i].pBankAccountNo == row.gBankAccountNumber) {
          this.bankdetailslist[i].gStatus = true;
        }
        else {
          this.bankdetailslist[i].gStatus = false;
  ...
  public editHandler({ dataItem }) {
    ...
    this.BankForm.controls.pBankName.setValue(dataItem.gBankName);
    this.BankForm.controls.pBankAccountNo.setValue(dataItem.gBankAccountNumber);
    this.BankForm.controls.pBankifscCode.setValue(dataItem.gBankIfscCode);
    this.BankForm.controls.pBankBranch.setValue(dataItem.gBankBranchName);
    this.BankForm.controls.ptypeofoperation.setValue(dataItem.ptypeofoperation);
    this.BankForm.controls.pIsprimaryAccount.setValue(dataItem.gStatus);
    this.BankForm.controls.pRefbankId.setValue(dataItem.gBankId);
    ...
    this.BankForm.controls.pStatusname.setValue(dataItem.gStatus);
    // this.BankForm.controls.pnameasperbank.setValue(dataItem.pNameAsPerBank);
  ...
  changeBankPriority(index: number) {
    debugger;
    this.BankForm['controls']['pbanktypeTypeChk'].setValue(this.bankdetailslist[index].gBankAccountNumber);
