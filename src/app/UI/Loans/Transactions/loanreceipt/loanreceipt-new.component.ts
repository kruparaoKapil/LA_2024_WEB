...
export class LoanreceiptNewComponent implements OnInit {
  ...
  public gstlist: any;
  public lstSaveEmireceiptdetails:any;
  ...
  calAmt(event, i, amountType) {
    ...
    if (this.dataOfParticulars[i].pParticularsname != 'INSTALMENT AMOUNT' && this.dataOfParticulars[i].pParticularsname != 'PRE EMI INTEREST') {
  ...
  saveLoanReceipt() {
    ...
    console.log( 'data of particulars',this.dataOfParticulars);
    for (let i = 0; i < this.dataOfParticulars.length; i++) {
    ...
    console.log('zeroReceipt',this.zeroReceipt);
    if (this.checkValidations(this.receiptForm, isvalid) && this.checkValidations(this.loandropForm, isvalid) && this.forNarrationErrorValid) {
      ...
      if (this.noaddbuttonforcharges && this.pChargename && this.enteredAmount) {
        this.lstSaveEmireceiptdetails.push(noaddemireceipt)
      }
      for (let i = 0; i < this.dataOfParticulars.length; i++) {
        if (this.zeroReceipt && this.zeroReceipt.length > 0) {
          for (let j = 0; j < this.zeroReceipt.length; j++) {
            if (this.zeroReceipt[j] != this.dataOfParticulars[i].pParticularsname) {
              emiReceipt = {
               ...
               console.log('lstSaveEmireceiptdetails',this.lstSaveEmireceiptdetails)
               let particularname=this.dataOfParticulars[i].pParticularsname;
               console.log('particularname',particularname)
               let duplicatedata:any=[];
               if(this.lstSaveEmireceiptdetails.length>0){
                 duplicatedata =this.lstSaveEmireceiptdetails.filter(function (data) {
             if(data.pParticularname==particularname){
              return data;
            }
                });
               }
               console.log('duplicate data count',duplicatedata)
            if(duplicatedata.length<=0){
              this.lstSaveEmireceiptdetails.push(emiReceipt);
            }
            }
          }
        } 
          ...
          if(this.lstSaveEmireceiptdetails.length>0){
          duplicatedata =this.lstSaveEmireceiptdetails.filter(function (data) {
            return data.pParticularname==this.dataOfParticulars[i].pParticularsname;
        });
          }
          console.log('duplicate data count',duplicatedata)
            if(duplicatedata.length<=0){
      ...
      let data = {
        ...
        lstSaveEmireceiptdetails:this.lstSaveEmireceiptdetails,
  ...
  selectCharge(event) {
    debugger;
    console.log(event,"For charge id");
    this.enteredAmount = null;
    this.enteredReceipt = null;
    this.penalityorcharges = 0;
