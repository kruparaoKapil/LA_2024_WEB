...
export class KycdocumentsnewComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._commonService._GetKYCUpdate().subscribe(data => {
      this.gridData = data;
      this.kycgrid = data
      console.log('this is grid data from service:', this.gridData);
      console.log('this is grid data  kycccc from service:', this.kycgrid);
    });
  }
  ...
  pIdProof_Change($event: any) {
    ...
    this.kycDocumentForm.controls.pDocReferenceno.setValue('');
    this.kYcDocumnetsErrorMessage['pDocReferenceno'] = '';
    if (this.documentname == 'PAN Card' || this.documentname == "PAN CARD") {
    ...
    if (this.checkValidations(this.kycDocumentForm, true)) {
      ...
      this.memberservice.CheckDocumentList(this.documentid, this.kycDocumentForm.controls.pDocReferenceno.value).subscribe(res => {
        if (!res) {
          ...
          // console.log('this is griddata forrr kyc:',this.gridData);
          this.kycgrid = [...this.kycgrid, this.kycDocumentForm.value]
          console.log('this is kycgridata forrr kyc:', this.kycgrid);
          this.gridData = this.kycgrid
          console.log('this is noramal grid data:', this.gridData);
          console.log('this is noramal gridkyyy data:', this.kycgrid); this.gridData = [... this.gridData, this.kycDocumentForm.value];
         ...
         // this.Datapassing(this.gridData)
          this.Datapassing(this.kycgrid)
          let a= this.Datapassing(this.kycgrid)
          console.log('printing datapassing data into console:',a);
          this.getFormKeys();
