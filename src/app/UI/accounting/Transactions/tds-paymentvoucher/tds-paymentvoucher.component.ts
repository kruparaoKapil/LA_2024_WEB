...
@Component({
  selector: 'app-tds-paymentvoucher',
  templateUrl: './tds-paymentvoucher.component.html',
  styles: []
})

export class TDSPaymentvoucherComponent implements OnInit {
...
;
  ...
  savebutton = "Save";
  tdsEnable:boolean = true;
  gstEnable:boolean = false;

  JSONdataItem: any = [];
  sectionAndPercentageList: any = [];
  tdsSectionList: any = [];
  tdsPerentage: any = [];
...
//this.tdsEnable = true;
  ...
  addppaymentsslistcontrols(): FormGroup {
    return this._FormBuilder.group({
      ...
      ppartyreferenceid: [''],
      ppartypannumber: [''],
      pistdsapplicable: [true, Validators.required],
      pgstno: [''],
      pTdsSection: [''],
      pTdsSectionname: [''],
      pTdsPercentage: [''],
...
if (event.target.checked) {
    ...
    this.tdsEnable = true;
    this.paymentVoucherForm['controls']['ppaymentsslistcontrols']['controls']['pistdsapplicable'].setValue(true)
    let ppartyname = this.paymentVoucherForm.get('ppaymentsslistcontrols').get('ppartyname').value;
    let checkexistingcount = 0;
    this.showtds = true;
  ...
  getTdsLedgerAccountHeads(){
    debugger;
    this._AccountingTransactionsService.getTdsLedgerAccountHeads().subscribe(json => {
      this.accHeadList = json;
    })
  }
...
debugger
    let pledgerid;
    if ($event != undefined) {
      pledgerid = $event.pACCOUNTID;
      this.pledgername = $event.pACCOUNTNAME;
    }
 ...
 debugger
    ...
    // this.pTdsSectionname = $event.pTdsSectionname;
    // this.paymentVoucherForm['controls']['ppaymentsslistcontrols']['controls']['pTdsSectionname'].setValue(this.pTdsSectionname);
    ...
    this._AccountingTransactionsService.getTdsectionsbyid($event.pTdsSection).subscribe(json => {
      this.tdsPerentage = json;
      this.paymentVoucherForm['controls']['ppaymentsslistcontrols']['controls']['pTdsSectionname'].setValue(this.tdsPerentage[0].pTdsSectionname);
      if(this.partyNamePan != 'NO PAN'){
      //this.paymentVoucherForm.get('ppaymentsslistcontrols').get('pTdsPercentage').setValue(this.tdsPerentage[0].pPanPercentage);

      this.paymentVoucherForm['controls']['ppaymentsslistcontrols']['controls']['pTdsSectionname'].setValue(this.tdsPerentage[0].pTdsSectionname);
      }
      if(this.partyNamePan == 'NO PAN'){
        this.tdsPerentage = [{'pPanPercentage' : 20}]
        //this.paymentVoucherForm.get('ppaymentsslistcontrols').get('pTdsPercentage').setValue(20);
  ...
  partyName_Change($event: any): void {
    ...
    if ($event != undefined) {
      ...
      this.partyNamePan = $event.ppartypannumber;
  ...
  getTdsections(){
    debugger;
    this._AccountingTransactionsService.getTdsections().subscribe(ress => {
      this.tdsSectionList = ress;
    })
  }
  getTdsPartyById(ppartyid){
    debugger;
    this._AccountingTransactionsService.getTdsPartyById(ppartyid).subscribe(response =>{
      this.sectionAndPercentageList = response;
      // this.paymentVoucherForm.get('ppaymentsslistcontrols').get('pTdsPercentage').setValue(this.sectionAndPercentageList[0].pTdsPercentage);

      // this.paymentVoucherForm.get('ppaymentsslistcontrols').get('pTdsSection').setValue(this.sectionAndPercentageList[0].pTdsSection);

      
      // this.paymentVoucherForm['controls']['ppaymentsslistcontrols']['controls']['pTdsSectionname'].setValue(this.sectionAndPercentageList[0].pTdsSectionname);
    })
  }
...
debugger
      ...
      if (json != null) {
        ...
        //let newdata = json.lstTdsSectionDetails.map(item => item.pTdsSection, item.pTdsSectionname)
        let newdata = json.lstTdsSectionDetails
  ...
  pamount_change() {
    ...
    let amountPaid = parseFloat(this.paymentVoucherForm['controls']['ppaymentsslistcontrols']['controls']['pactualpaidamount'].value.toString().replace(/,/g, ""));
    let cashValidation = this.cashBalance.split(' ');
    if (cashValidation[0] == "0" && this.paymentVoucherForm.controls.pmodofpayment.value == 'CASH') {
      this._commonService.showWarningMessage('Cash Balance Not Available');
      this.paymentVoucherForm['controls']['ppaymentsslistcontrols']['controls']['pactualpaidamount'].setValue('');
      return;
    }

    let cashvalue = Number(cashValidation[0].toString().replace(/,/g, ""));

    if (this.paymentVoucherForm.controls.pmodofpayment.value == 'CASH') {
      if (amountPaid > 9999 || amountPaid > cashvalue) {
        this._commonService.showWarningMessage('Cash Payment Dosenot  Accept More Than 9,999');
        this.paymentVoucherForm['controls']['ppaymentsslistcontrols']['controls']['pactualpaidamount'].setValue('');
        return
      }
    }

    let paidAmount = this.paymentVoucherForm['controls']['ppaymentsslistcontrols']['controls']['pactualpaidamount'].value;

    let subBal = this.subledgerBalance.split(' ');

    let actSubBal =  Number(subBal[0].toString().replace(/,/g, ""));

    if (this.pledgername == 'OUTSTANDING PROMOTER SALARY') {  

    if (amountPaid > actSubBal) {
        this._commonService.showWarningMessage('Paid Amount Shouldnot Be Greaterthan Sub Ledger Balance');
        this.paymentVoucherForm['controls']['ppaymentsslistcontrols']['controls']['pactualpaidamount'].setValue('');
        return
    }
  }
  ...
  claculategsttdsamounts() {
    ...
    try {
      ...
      this.paymentVoucherForm.get('ppaymentsslistcontrols').get('pisgstapplicable').setValue(true)
...
debugger
    ...
    if (this.validateaddPaymentDetails()) {
      ...
      if(this.subledgeraccountslist.length == 0){
        this._commonService.showWarningMessage('Please Add Sub Category');
        this.addbutton = "Add";
...
if(this.paymentslist.length<=0){
  ...
  return;
    ...
    if (this.validatesavePaymentVoucher()) {
      ...
      this.paymentVoucherForm['controls']['ppaymentsslistcontrols']['controls']['ppartypannumber'].setValue(this.partyNamePan);
      ...
      this._AccountingTransactionsService.saveTDSPaymentVoucher(data).subscribe(res => {
        debugger;
        if (res[0] == 'TRUE') {
          ...
          this._routes.navigate(['/TdsPaymentvoucherView']);

          window.open('/#/TdsPaymentVoucherReports?id=' + btoa(res[1] + ',' + 'TDS Payment Voucher'));
...
debugger
    try {
      ...
      if (journalentryamount > 0) {
        ...
        this.partyjournalentrylist.push(dataobject);
        let pamount1 = this.paymentslist.reduce((sum, item) => sum + parseFloat((item.ptdsamount).replace(/,/g, "")), 0);
        dataobject =  { type: 'Payment Voucher', accountname: 'TDS Amount', debitamount: '', creditamount: this._commonService.currencyformat(pamount1) }
        this.partyjournalentrylist.push(dataobject);
      }
