...
@Component({
  ...
  template: `
    <div class="page">
      ...
      <app-data-grid
        title="All Referral Agents"
        [rows]="rows()"
        [columns]="columns"
        [loading]="loading()"
        exportFilename="referral-agents"
        (rowDblClick)="edit($event)"
      />

      <app-dialog
        [(visible)]="dialogOpen"
        [header]="editing() ? 'Edit' : 'New Referral Agent'"
        width="640px"
        [showOk]="false"
        [showCancel]="false"
      >
        <div class="grid">
          ...
          <div class="field">
            ...
            <p-inputNumber
              [ngModel]="form().pCommissionPercent ?? null"
              (ngModelChange)="patch({ pCommissionPercent: $event ?? undefined })"
              [min]="0"
              [max]="100"
              [maxFractionDigits]="2"
            />
...
export class ReferralAgentComponent {
  private readonly svc = inject(ReferralAgentService);
  private readonly auth = inject(AuthStore);
  private readonly toast = inject(ToastService);
  private readonly loader = inject(LoaderService);

  protected readonly rows = signal<ReferralAgentRow[]>([]);
  protected readonly loading = signal<boolean>(false);
  protected readonly busy = signal<boolean>(false);
  protected readonly dialogOpen = signal<boolean>(false);
  protected readonly form = signal<ReferralAgentRow>(empty());
  protected readonly editing = computed(() => this.form().pAgentID > 0);
  ...
  protected readonly columns: DataGridColumn<ReferralAgentRow>[] = [
  ...
  protected edit(row: ReferralAgentRow): void {
    this.form.set({ ...row });
    this.dialogOpen.set(true);
  }
  protected patch(part: Partial<ReferralAgentRow>): void {
