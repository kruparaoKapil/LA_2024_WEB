...
disabletransactiondate = false;
  ...
  ngOnInit() {
   ...
   this.psavingaccname='';
     ...
     this.pissavingspayinapplicable='';
     this.receiptcount='';
...
this.bankshowhide = false;
  ...
  SavingAccountNumber_Change(event){
    ...
    if(event){
     ...
     let satransdate=this._commonService.formatDateFromDDMMYYYY(this.savingaccountDetails.ptransdate);
     this.dpConfig1.minDate=satransdate;
  ...
  AmountReceived_Change(){
    ...
    let amountreceived=this._commonService.removeCommasForEntredNumber(this.savingtransdetails.controls.pReceivedamount.value);
    let maxdepositamount=this._commonService.removeCommasForEntredNumber(this.psavingmaxdepositamount);
    let mindepositamount=this._commonService.removeCommasForEntredNumber(this.psavingmindepositamount);
    let minopeningamount=this._commonService.removeCommasForEntredNumber(this.pminopenamount)
    if(amountreceived<=0){
       this._commonService.showWarningMessage('Received Amount must be greater than Zero.');
            this.savingtransdetails.controls.pReceivedamount.setValue('');
            return isValid=false;
    }
    let Issavingspayinapplicable=this.pissavingspayinapplicable;
     if(parseInt(this.receiptcount)==0 && minopeningamount>0){
          if(amountreceived<minopeningamount){
            this._commonService.showWarningMessage('Received Amount must be greater than or equal to Min Opening Amount	('+ this._commonService.currencyformat(this.pminopenamount) +')');
            this.savingtransdetails.controls.pReceivedamount.setValue('');
            return isValid=false;
          }
      }
    if(Issavingspayinapplicable){
    if(maxdepositamount>0){
          if(amountreceived>maxdepositamount || amountreceived<mindepositamount){
            this._commonService.showWarningMessage('Received Amount must be Between or equal to Min. Deposit Value and Max. Deposit Value');
            this.savingtransdetails.controls.pReceivedamount.setValue('');
            return isValid=false;
          }
        }
