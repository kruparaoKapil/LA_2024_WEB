...
@Component({
  selector: 'app-share-issue-report',
  templateUrl: './share-issue-report.component.html',
  styles: []
})
export class ShareIssueReportComponent implements OnInit {
ShareIssueReportForm: FormGroup;
 showbetween:boolean=false;
  ...
  ngOnInit() {
    this.ShareIssueDetailsList=[];
    this.ShareIssueReportForm = this.FB.group({
    ...
    this.ShareIssueReportForm['controls']['pToMonthOf'].setValue(new Date());
  ...
  DatetypeChange(type) {
   ...
   this.ShareIssueDetailsList=[];
    if (type == "ASON") {
      ...
      this.ShareIssueReportForm['controls']['pFromMonthOf'].setValue(new Date());
      this.ShareIssueReportForm['controls']['pToMonthOf'].setValue(new Date());
  ...
  FromDateChange($event: any) {
    ...
    this.fromdate = this.datepipe.transform(this.frommonthof, "yyyy-MM-dd");
     if ( this.ShareIssueReportForm.controls.datecheck.value== "BETWEEN") {
       this.Change_Date();
     }
 ...
 Change_Date() {
    debugger;
    let fromdate = this.datepipe.transform(this.ShareIssueReportForm['controls']['pFromMonthOf'].value, "dd/MM/yyyy");
    let todate = this.datepipe.transform(this.ShareIssueReportForm['controls']['pToMonthOf'].value, "dd/MM/yyyy");
...
GetShareIssueDetails(){
  debugger;

  let datecheck=this.ShareIssueReportForm.controls.datecheck.value;
  let fromdate=this.datepipe.transform(this.ShareIssueReportForm.controls.pFromMonthOf.value,'yyyy-MM-dd');
  let todate=this.datepipe.transform(this.ShareIssueReportForm.controls.pToMonthOf.value,'yyy-MM-dd');
  this._LAReportsService.GetShareIssueDetails(datecheck,fromdate,todate).subscribe(result=>{
    debugger;
    if(result){
    this.ShareIssueDetailsList=result;
    }
  })
}
  ...
  pdf() {
    ...
    let type;
    let gridheaders;
    let reportname = "Share Issue Report";
   
    gridheaders = ["Share Account Number","Member Name","No. of Shares",  "Share Value","Total Share Value", "Share Issue Date","Scheme Name"];
    
    let colWidthHeight = {
       0: { cellWidth: 'auto' }, 1: { cellWidth: 'auto' }, 2: { cellWidth: 'auto' ,halign: 'right'  }, 3: { cellWidth: 'auto',halign: 'right'}, 4: { cellWidth: 'auto',halign: 'right'}, 5: { cellWidth: 'auto',halign: 'center'}, 6: { cellWidth: 'auto' }
    }

    // let data = "true"
    // if (data == "true") {
    //   debugger;
    //     this.returndata = this._CommonService._getGroupingGridExportData(this.ShareIssueDetailsList, "pMembername", false)
    // }
    // else {
        this.returndata = this.ShareIssueDetailsList
//}
    debugger;
    this.returndata.forEach(element => {  
      debugger;
        if (element.group !== undefined) {
            temp = [element.group,element.paccountno,element.pMembername ,element.pNoofshares, this._CommonService.currencyformat(element.pSharevalue),this._CommonService.currencyformat(element.pTotalAmount),element.this.datepipe.transform(element.pshares_issue_date,'dd-MMM-yyyy'),element.pSchemename];
        }
        else {
            temp = [element.paccountno,element.pMembername ,element.pNoofshares,this._CommonService.currencyformat( element.pSharevalue),this._CommonService.currencyformat(element.pTotalAmount), this.datepipe.transform(element.pshares_issue_date,'dd-MMM-yyyy'),element.pSchemename];
        } 
