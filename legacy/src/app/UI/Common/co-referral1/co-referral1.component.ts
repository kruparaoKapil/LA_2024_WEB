...
declare let $: any
@Component({
  selector: 'app-co-referral1',
  templateUrl: './co-referral1.component.html',
  styles: []
})
export class CoReferral1Component implements OnInit {
  ...
  savingtheform() {
    ...
    this.loading = true;
    this._CoReferralService.SaveReferralData1(data).subscribe(json => {
      debugger
      if (json['referralsStatu']) {
        ...
        if (this.returnurl == '') {
          ...
          $('.nav-item a[href="#' + str + '"]').tab('show');
            ...
            let receipt = btoa(json['maturity_Details_ID'] + ',' + 'MaturityPayment');
            window.open('/#/MaturityPaymentPreview?id=' + receipt);
