  ...
  ngOnInit() {
   ...
   // return;
    ...
    // if (this._route.snapshot.params['id']) {
     // this._CoJointmemberService.getMemberNomineeDetails().subscribe(message => { this.nomineeList = message; });
    this._CoJointmemberService.getMemberNomineeDetails().subscribe((response) => {
      debugger
    //  this.nomineeList = [];
      //this._CoJointmemberService._SetnomineeList(this.nomineeList);
      let a = response;
      if (a !== undefined || a !== null) {
       ...
       //   let count = this.nomineeList.filter(data => data.pisprimarynominee == true)
          this._CoJointmemberService._SetnomineeList(this.nomineeList);
          this.checkedlist = this.nomineeList;
    ...
    //   debugger
      if (this.nomineeDetailsEditData != null) {
        ...
        this.nomineeList = [...this.nomineeDetailsEditData];
       // let count = this.nomineeList.filter(data => data.pisprimarynominee == true)
        this._CoJointmemberService._SetnomineeList(this.nomineeList);
        this.checkedlist = this.nomineeList
      }
        ...
        if (this.validateNominee()) {
          if (this.validatepercentage() && this.nomineeForm.controls.pPercentage.value <= 100) {
            ...
            if (this.nomineeForm.controls.pPercentage.value == 0) {
              ...
              let pdateofbirth=this.nomineeForm.controls.pdateofbirth.value;
              this.nomineeForm.controls.pdateofbirth.setValue(this._commonService.getFormatDate(pdateofbirth));
let nomineeFormvalue=[];
nomineeFormvalue.push(this.nomineeForm.value);
              this.nomineeList=[...this.nomineeList,...nomineeFormvalue];
              //let count = this.nomineeList.filter(data => data.pisprimarynominee == true)
              this._CoJointmemberService._SetnomineeList(this.nomineeList);
              this.checkedlist = this.nomineeList;
            //  this.nomineeList=[];
              //this.nomineeList=[...this.checkedlist];
              ...
              this.nomineeForm = this._FormBuilder.group({
                ...
                pAge: [''],
                pTypeChk: [''],
                pcontactno: ['',[Validators.minLength(10)]],
                pisprimarynominee: [],
                pidprooftype: [''],
                pDocumentGroup: [''],
                pDocumentGroupId: [''],
  ...
  removeHandler(event) {
   ...
   // this.checkedlist.splice(event.rowIndex, 1);
    this.nomineeDetailsEditData = [...this.nomineeList];
  }
  ...
  Percentagechangeingrid($event, dataItem, rowindex) {
    ...
    $event=0;
    this.nomineeList.filter(function (df) {
      if(df.pnomineename==dataItem.pnomineename){
        df.pPercentage=0;
      }
      });
