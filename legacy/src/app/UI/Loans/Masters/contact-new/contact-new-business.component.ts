 ...
 @ViewChild(DatatableComponent, { static: false }) table: DatatableComponent;
  ...
  loadEditDetails() {
    ...
    if (this.formtype == 'Update' && this.contactType != 'Individual') {
      ...
      console.log('kyc update details form json:',json);
        try {
          if (json != null) {
  ...
  addcontactdetails(): void {
    try {
      ...
      if(contacdata.contactname=='' || contacdata.contactname==null){
        this._commonService.showWarningMessage('Select Contact');
        return;
      }
      if(businesscontrol.controls.designationname.value=='' || businesscontrol.controls.designationname.value==null){
        this._commonService.showWarningMessage('Select Designation');
        return;
      }
...
debugger;
  ...
  saveContactDetails(): void {
    debugger
    try {
      if(this.contactForm.controls.pBusinessEntityContactno.value==''|| this.contactForm.controls.pName.value==''){
...
if(this.addressdeletedrows.length>0){
  ...
  Pincode(){
   ...
   if(PinCode.length==6){
     ...
     let addres=control.controls.pAddress1.value + "," + control.controls.pAddress2.value
